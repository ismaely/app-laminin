...
router.post('/login', async(req, res) => {
       ...
       const resultado = await Consultor.findOne({email: req.body.email},
           (err, user) =>{
            if (!user || !user.comparePassword(req.body.password)) {
            return res.status(400).json({ message: 'Autenticação falhou...' });
            }
            return res.json({ token: jwt.sign({ email: user.email, nome: user.nome, _id: user._id }, process.env.SECRET) });
        });
...
try {
    const pesquisa = await History.find({$or: [{mercado: req.params.nome},{nome_produto: req.params.nome}]})
    if(pesquisa == null){
